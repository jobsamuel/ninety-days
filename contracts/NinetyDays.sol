...
contract NinetyDays {
  ...
  event ChallengeEntry(address indexed participant, uint256 valueCommitted, uint256 daysCommitted);
  event ChallengeExit(address indexed participant, uint256 valueReturned, uint256 daysLeft);
  event ChallengeUpdated(address indexed participant, uint256 daysCommitted, uint256 daysLeft);
  event ChallengeFinished(address indexed participant, uint256 daysCommitted, uint256 daysLeft);
  event BonusDistributed(uint256 activeParticipants, uint256 bonusPerParticipant);
  ...
  */
  ...
  modifier isAdmin() {
    require(msg.sender == admin, "You are not allowed to perform this action.");
    _;
  }

  modifier isOpen() {
    require(allowChallengeEntries, "Challenge is closed.");
    require(!includedInChallenge[msg.sender], "You already have entered the challenge.");
    _;
  }
  ...
  */
  function enterChallenge(uint256 durationInDays) isOpen payable external returns (bool) {
    if (durationInDays > 0 && durationInDays < 90) {
      require(msg.value >= (minimumChallengeEntryPriceInWei * 5), "Minimum entry price is 0.05");
    } else {
      require(msg.value >= minimumChallengeEntryPriceInWei, "Minimum entry price is 0.01");
    }
    ...
    challengers[msg.sender] = Challenger(msg.sender, price, daysCommitted, block.timestamp, daysCommitted);
  ...
  */
  function exitChallenge() participantOnly payable external returns (bool) {
    require(challengers[msg.sender].daysLeft >= 1, "You already have finished the challenge.");
    ...
    emit ChallengeExit(msg.sender, amountToBeReturned, challengers[msg.sender].daysLeft);
  ...
  */
  function updateDaysLeftInChallenge() participantOnly external returns (bool) {
    require(block.timestamp >= challengers[msg.sender].lastCommit + 86400, "Can not update temporally.");

    // Challenge can ONLY be updated between 24h AND 48h from last update.
    bool canUpdate = block.timestamp < challengers[msg.sender].lastCommit + 86400 * 2;
    ...
    if (challengers[msg.sender].daysLeft >= 1 && canUpdate) {
      ...
      emit ChallengeUpdated(msg.sender, challengers[msg.sender].daysCommitted, challengers[msg.sender].daysLeft);
    }
  ...
  */
  function finishChallenge(address participant) private returns (bool) {
    ...
    emit ChallengeFinished(participant, challengers[participant].daysCommitted, challengers[participant].daysLeft);
   ...
   */
  function distributeBonusToActiveParticipants(uint256 amount) private returns (bool) {
    ...
    */
  function forceBonusDistribution() isAdmin external {
  ...
  /**
    ...
    */
  function takeFeesOut() isAdmin payable external {
  ...
  /**
    ...
    * Emits a {Transfer} event.
    */
  function withdraw() isAdmin payable external {
  ...
  /**
    ...
    */
  function updateChallengeAccess() isAdmin external {
  ...
  /**
    ...
    */
  function transferAdministration(address newAdmin) isAdmin external {
  ...
  function getChallengeBalanceOf(address participant) external view returns (uint256) {
    return balances[participant];
  }

  function isIncludedInChallenge(address participant) external view returns (bool) {
    return includedInChallenge[participant];
  }

  function calculateEntryChallengeFee(uint256 amount) private view returns (uint256) {
    return (amount * entryFee) / 100; 
  }

  function calculateExitChallengeFee(uint256 amount) private view returns (uint256) {
    return (amount * exitFee) / 100;
  }

  function getChallenger(address participant) external view returns (uint256, uint256, uint256, uint256) {
